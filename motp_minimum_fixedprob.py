# Filename: motp_minimum_fixedprob.py
# Implementation of the algorithm using MINIMUM VALUE instead of arithmetic/geometric mean.
# It considers the minimum value across objectives for each cell, a fixed probability matrix,
...
def minimum_value_matrix(matrices):
    arrs = [np.array(m, dtype=float) for m in matrices]
    stacked = np.stack(arrs, axis=2)
    min_matrix = np.min(stacked, axis=2)
    return min_matrix

def fixed_probability_matrix(min_matrix):
    cmm = np.array(min_matrix, dtype=float)
    CMmax = np.max(cmm)
    diff = CMmax - cmm
    total = np.sum(diff)
    if total == 0:
        return np.full(cmm.shape, 1.0 / cmm.size)
    P = diff / total
    return P
...
def allocate_from_line(choice, index, P, min_mat, supply, demand, active_rows, active_cols):
    if choice == 'row':
        i = index
        ...
        best_j = cand_cols[0]
        best_cost = min_mat[i, best_j]
        for j in cand_cols:
            if min_mat[i, j] < best_cost or (min_mat[i, j] == best_cost and j < best_j):
                best_cost = min_mat[i,j]
                best_j = j
        ...
        # Among candidates with max probability, select the one with minimum cost
        best_i = cand_rows[0]
        best_cost = min_mat[best_i, j]
        for i in cand_rows:
            if min_mat[i, j] < best_cost or (min_mat[i, j] == best_cost and i < best_i):
                best_cost = min_mat[i,j]
                best_i = i
...
def run_algorithm(objective_matrices, supply, demand, debug=False):
    ...
    min_mat = minimum_value_matrix(objective_matrices)
    P = fixed_probability_matrix(min_mat)
    rpen, cpen = row_col_penalties(P)
    rows, cols = min_mat.shape
    ...
    while np.any(supply > 0) and np.any(demand > 0):
        ...
        sel = allocate_from_line(choice, idx, P, min_mat, supply, demand, active_rows, active_cols)
        if sel is None:
            if choice == 'row':
        ...
        if debug:
            print(f"  Allocating: S{i+1} → D{j+1}: {qty} units (min cost: {min_mat[i,j]:.2f})")
