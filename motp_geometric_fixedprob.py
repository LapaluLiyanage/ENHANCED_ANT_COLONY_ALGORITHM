# Filename: motp_geometric_fixedprob.py
# Implementation of the algorithm exactly as presented in the provided paper.
# It computes geometric mean across objectives, a fixed probability matrix,
...
def geometric_mean_matrix(matrices):
    ...
    with np.errstate(invalid='ignore'):
        gm = np.prod(stacked, axis=2) ** (1.0 / stacked.shape[2])
    return gm

def fixed_probability_matrix(geom_matrix):
    cgmm = np.array(geom_matrix, dtype=float)
    CGMmax = np.max(cgmm)
    diff = CGMmax - cgmm
    total = np.sum(diff)
    if total == 0:
        return np.full(cgmm.shape, 1.0 / cgmm.size)
    P = diff / total
    return P
...
def select_highest_unique_penalty(rpen, cpen, active_rows, active_cols):
    ...
    if max_r >= max_c:
        ...
        if candidates.size == 0:
            return ('col', int(np.where(cp == max_c)[0][0]))
        return ('row', int(candidates[0]))
    else:
        candidates = np.where(cp == max_c)[0]
        return ('col', int(candidates[0]))

def allocate_from_line(choice, index, P, geom, supply, demand, active_rows, active_cols):
    if choice == 'row':
        i = index
        ...
        best_j = cand_cols[0]
        best_cost = geom[i, best_j]
        for j in cand_cols:
            if geom[i, j] < best_cost:
                best_cost = geom[i,j]
                best_j = j
...
def run_algorithm(objective_matrices, supply, demand):
    supply = np.array(supply, dtype=int).copy()
    demand = np.array(demand, dtype=int).copy()
    geom = geometric_mean_matrix(objective_matrices)
    P = fixed_probability_matrix(geom)
    ...
    while np.any(supply > 0) and np.any(demand > 0):
        ...
        if choice is None:
            break
        sel = allocate_from_line(choice, idx, P, geom, supply, demand, active_rows, active_cols)
        if sel is None:
            if choice == 'row':
...
if __name__ == "__main__":
    ...
    result = run_algorithm(objective_matrices, supply, demand)
    
    print("\n" + "="*60)
    print("RESULTS")
    print("="*60)
    print("\nGeometric Mean Matrix:")
    print(result['geom_matrix'])
    print("\nProbability Matrix:")
